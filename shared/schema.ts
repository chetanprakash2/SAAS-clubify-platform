...
export interface IMeeting extends BaseDocument {
  ...
  participants: string[]; // Array of user IDs currently in the meeting
  maxParticipants?: number;
  isVoiceOnly: boolean; // true for voice-only, false for video (disabled for stability)
}

// Meeting Chat Message interfaces
export interface IMeetingMessage extends BaseDocument {
  meetingId: string;
  senderId: string;
  content: string;
  messageType: "text" | "system"; // system for join/leave notifications
}
...
export const meetingSchema = new Schema<IMeeting>({
  ...
  isVoiceOnly: { type: Boolean, default: true },
}, {
  timestamps: true,
  collection: 'meetings'
});

export const meetingMessageSchema = new Schema<IMeetingMessage>({
...
export type MeetingMessage = IMeetingMessage;
