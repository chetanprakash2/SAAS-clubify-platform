...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Meeting routes
  app.post('/api/clubs/:clubId/meetings', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.id;
      ...
      const membership = await storage.getClubMembership(userId, clubId);
      if (!membership) {
        return res.status(403).json({ message: "Only club members can create meetings" });
      }
  ...
  app.get('/api/clubs/:clubId/meetings', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.id;
      const clubId = req.params.clubId;
      ...
      const membership = await storage.getClubMembership(userId, clubId);
      if (!membership) {
        return res.status(403).json({ message: "Only club members can view meetings" });
      }
  ...
  app.get('/api/meetings/:id', isAuthenticated, async (req: any, res) => {
    try {
      ...
      // Check if user is a member of the club
      const membership = await storage.getClubMembership(userId, meeting.clubId);
      if (!membership) {
        return res.status(403).json({ message: "Only club members can view this meeting" });
      }
  ...
  app.patch('/api/meetings/:id', isAuthenticated, async (req: any, res) => {
    try {
      ...
      // Check if user is admin of the club or meeting creator
      const isAdmin = await storage.isClubAdmin(userId, meeting.clubId);
      const isCreator = meeting.createdBy === userId;
      
      if (!isAdmin && !isCreator) {
        return res.status(403).json({ message: "Only admins or meeting creators can edit meetings" });
      }
      
      const updateData = updateMeetingSchema.parse(req.body);
  ...
  app.post('/api/meetings/:id/join', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.id;
      const meetingId = req.params.id;
      ...
      const membership = await storage.getClubMembership(userId, meeting.clubId);
      if (!membership) {
        return res.status(403).json({ message: "Only club members can join meetings" });
      }
      ...
      await storage.createMeetingMessage({
        meetingId,
        senderId: userId,
        content: `${req.user.firstName || 'User'} joined the meeting`,
        messageType: 'system'
      });
  ...
  app.post('/api/meetings/:id/leave', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.id;
      const meetingId = req.params.id;
      ...
      if (!meeting.participants.includes(userId)) {
        return res.status(400).json({ message: "Not currently in this meeting" });
      }
      ...
      await storage.createMeetingMessage({
        meetingId,
        senderId: userId,
        content: `${req.user.firstName || 'User'} left the meeting`,
        messageType: 'system'
      });
  ...
  app.post('/api/meetings/:id/start', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.id;
      const meetingId = req.params.id;
      ...
      if (!isAdmin && !isCreator) {
        return res.status(403).json({ message: "Only admins or meeting creators can start meetings" });
      }
      ...
      // Create system message for meeting start
      await storage.createMeetingMessage({
        meetingId,
        senderId: userId,
  ...
  app.post('/api/meetings/:id/end', isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (!isAdmin && !isCreator) {
        return res.status(403).json({ message: "Only admins or meeting creators can end meetings" });
      }
      ...
      // Create system message for meeting end
      await storage.createMeetingMessage({
        meetingId,
        senderId: userId,
  ...
  // Meeting chat routes
  app.get('/api/meetings/:id/messages', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.id;
      const meetingId = req.params.id;
      ...
      const membership = await storage.getClubMembership(userId, meeting.clubId);
      if (!membership) {
        return res.status(403).json({ message: "Only club members can view meeting chat" });
      }
  ...
  app.post('/api/meetings/:id/messages', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.id;
      const meetingId = req.params.id;
      ...
      if (!meeting.participants.includes(userId)) {
        return res.status(403).json({ message: "Only meeting participants can send messages" });
      }
      
      const messageData = insertMeetingMessageSchema.parse({
        ...req.body,
        meetingId,
        senderId: userId,
      });
      
      const message = await storage.createMeetingMessage(messageData);
      res.json(message);
    } catch (error) {
