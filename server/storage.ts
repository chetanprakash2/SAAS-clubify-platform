...
import {
  ...
  type MeetingMessage as MeetingMessageType,
  type InsertClub,
  type UpdateClub,
...
export interface IStorage {
  ...
  // Meeting operations
  createMeeting(meeting: any): Promise<MeetingType>;
  getClubMeetings(clubId: string): Promise<(MeetingType & { creator: UserType })[]>;
  getMeeting(id: string): Promise<MeetingType | undefined>;
  updateMeeting(id: string, data: any): Promise<MeetingType>;
  ...
  createMeetingMessage(message: any): Promise<MeetingMessageType>;
  getMeetingMessages(meetingId: string): Promise<(MeetingMessageType & { sender: UserType })[]>;
}
...
export class DatabaseStorage implements IStorage {
  ...
  async getUserClubs(userId: string): Promise<(ClubType & { membership: ClubMembershipType })[]> {
    ...
    return memberships.map(membership => {
      ...
      return club ? { ...club, membership } as any : null;
    }).filter(Boolean) as (ClubType & { membership: ClubMembershipType })[];
  }
  ...
  async getClubMembers(clubId: string): Promise<(ClubMembershipType & { user: UserType })[]> {
    ...
    try {
      ...
      return memberships.map(membership => {
        ...
        return user ? { ...membership, user } as any : null;
      }).filter(Boolean) as (ClubMembershipType & { user: UserType })[];
    } catch (error) {
  ...
  async getClubJoinRequests(clubId: string): Promise<(JoinRequestType & { user: UserType })[]> {
    ...
    return requests.map(request => {
      ...
      return user ? { ...request, user } as any : null;
    }).filter(Boolean) as (JoinRequestType & { user: UserType })[];
  }
  ...
  async getClubAnnouncements(clubId: string): Promise<(AnnouncementType & { author: UserType })[]> {
    ...
    return announcements.map(announcement => {
      ...
      return author ? { ...announcement, author } as any : null;
    }).filter(Boolean) as (AnnouncementType & { author: UserType })[];
  }
  ...
  async getClubEvents(clubId: string): Promise<(EventType & { creator: UserType })[]> {
    ...
    return events.map(event => {
      ...
      return creator ? { ...event, creator } as any : null;
    }).filter(Boolean) as (EventType & { creator: UserType })[];
  }
  ...
  async getClubTasks(clubId: string): Promise<(TaskType & { creator: UserType; assignee?: UserType })[]> {
    ...
    return tasks.map(task => {
      ...
      return creator ? { ...task, creator, assignee } as any : null;
    }).filter(Boolean) as (TaskType & { creator: UserType; assignee?: UserType })[];
  }
  ...
  async getClubChatMessages(clubId: string, limit = 50): Promise<(ChatMessageType & { sender: UserType })[]> {
    ...
    return messages.map(message => {
      ...
      return sender ? { ...message, sender } as any : null;
    }).filter(Boolean) as (ChatMessageType & { sender: UserType })[];
  }
  ...
  // Meeting operations
  async createMeeting(meeting: any): Promise<MeetingType> {
    if (this.useMemory()) {
      const meetingId = this.memoryStore.generateId();
      const newMeeting: MeetingType = {
        ...
        isVoiceOnly: meeting.isVoiceOnly !== undefined ? meeting.isVoiceOnly : true,
        createdAt: new Date(),
        updatedAt: new Date(),
      } as MeetingType;
      return newMeeting;
  ...
  async getClubMeetings(clubId: string): Promise<(MeetingType & { creator: UserType })[]> {
    if (this.useMemory()) {
      return [];
    }
    const meetings = await Meeting.find({ clubId })
      .sort({ createdAt: -1 })
      .lean();
    
    // Get creators manually to avoid ObjectId casting issues
    const creatorIds = meetings.map(m => m.createdBy);
    const creators = await User.find({ id: { $in: creatorIds } }).lean();
    
    return meetings.map(meeting => {
      const creator = creators.find(u => u.id === meeting.createdBy);
      return creator ? { ...meeting, creator } as any : null;
    }).filter(Boolean) as (MeetingType & { creator: UserType })[];
  }
  ...
  async updateMeeting(id: string, data: any): Promise<MeetingType> {
    if (this.useMemory()) {
      const meeting = { ...data, _id: id, updatedAt: new Date() } as MeetingType;
      return meeting;
    }
  ...
  // Meeting message operations
  async createMeetingMessage(message: any): Promise<MeetingMessageType> {
    if (this.useMemory()) {
      const messageId = this.memoryStore.generateId();
  ...
  async getMeetingMessages(meetingId: string): Promise<(MeetingMessageType & { sender: UserType })[]> {
    if (this.useMemory()) {
      return [];
    }
    ...
    return messages.map(message => {
      const sender = senders.find(u => u.id === message.senderId);
      return sender ? { ...message, sender } as any : null;
    }).filter(Boolean) as (MeetingMessageType & { sender: UserType })[];
  }
}
